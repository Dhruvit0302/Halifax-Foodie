const admin = require('firebase-admin');
const functions = require('firebase-functions')
const cors = require('cors')({ origin: true });

admin.initializeApp({
  credential:admin.credential.cert({
  "type": "service_account",
  "project_id": "questionanswer-b831c",
  "private_key_id": "68cc31ef4c95ab2515d2f0343814972737fa9e51",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "firebase-adminsdk-sqbxq@questionanswer-b831c.iam.gserviceaccount.com",
  "client_id": "100730592537287290544",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-sqbxq%40questionanswer-b831c.iam.gserviceaccount.com"
  })
});

const db = admin.firestore();
const collection = "QuestionAnswer";

exports.qNaLoginFunction = functions.https.onRequest(async (req, res) => {
  await cors(req, res, async () => {
    try{
      const rdata = await db.collection(collection).doc(JSON.parse(req.body.body).email.toString()).get();
      if(rdata.exists){
        if(rdata.data().answer.toLowerCase()==JSON.parse(req.body.body).answer.toLowerCase()){
          res.status(200).json({data: true,status: true, message: 'Success'})
        } else {
          res.status(200).json({data:false, status: false, message: 'Invalid answer!'})
        }
      } else {
        res.status(200).json({status: false, message: 'Something went wrong'})  
      }
    } catch(e){
       res.status(200).json({status: false, message: 'Something went wrong'})
    }
  })
});
